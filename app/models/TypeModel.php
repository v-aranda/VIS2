...
class TypeModel
{
    ...
    public function readByProduct($prd_id)
    {   
        
        try {
            $stmt = $this->pdo->prepare("SELECT elementType.ety_id AS ety_id, elementType.ety_name AS ety_name FROM prd_ety INNER JOIN elementType ON prd_ety.ety_id = elementType.ety_id WHERE prd_ety.prd_id = :id");
            $stmt->bindParam(':id', $prd_id);
            $stmt->execute();
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Erro ao ler registro por ID: " . $e->getMessage());
