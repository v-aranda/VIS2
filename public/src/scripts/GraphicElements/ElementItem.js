...
export default class ElementItem {
  ...
  async createItem() {
    ...
    if (this.element.typeOfElement == '-1') {
      ...
      let positionPreloadedName
      if(this.element.elementPosition != '-1'){
         positionPreloadedName = this.positionsList.filter(position => this.element.elementPosition == position.pos_id)[0].pos_name.toUpperCase()
      }else{
         positionPreloadedName = "Selecionar Posição"
      }
      const positionModalButton = new PositionButton(
        `positionButton_${this.index}`,
        ["btn-secondary", "btn", "ml-3", "col-4"],
